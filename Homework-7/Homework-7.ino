...
byte maxMatrixBrightness = 16;

/// LCD Brightness and eeprom location
byte lcdBrightness;
byte lcdBrightnessByte = 1;
const byte lcdMultiplier = 10;
byte maxLCDBrightness = 26;
...
/// Rules for the game
const byte numberOfMessages = 18;
const String howToPlay[numberOfMessages] = { "Bomberman Rules!", "You are a", "Bomberman and", "your goal is to", "destroy all the",
                                             "walls on the map", "without dying", "using special", "bombs. You can", "move using the", "joystick, place",
                                             "bombs with the", "main button, and", "switch between", "the bombs using", "the right and", "left buttons.", "Good Luck!" };
...
const String settingsOptions[numberOfSettings] = { "LCD Light", "Matrix Light", "Sound", "Difficulty", "Exit" };

/// Sound variables and eeprom
bool soundState = true;
byte soundStateByte = 3;
const String OnAndOff[2] = { "Off", "On" };

/// numberOfDifficulty variables and eeprom
const byte numberOfDifficulties = 3;
const String difficulties[numberOfDifficulties] = { "Easy", "Medium", "Hard" };
byte currentDifficulty = 1;
byte currentDifficultyByte = 4;
float mapCovrageDifficulty[numberOfDifficulties] = { 0.5, 0.6, 0.75 };

/// Welcome message
const String welcomeMessage[2] = { "Welcome to", "Casoleta gaming!" };
bool introOnce = false;


/// Meniu variables
const int delayBetweenMessages = 1250;
...
int playerScore[numberOfHighscores] = { 3, 2, 1 };
...
/// Special bombs
bool dirtBomb = false;
byte numberOfDirtBombsDifficulty[numberOfDifficulties] = { 6, 4, 2 };
  ...
  if (joyMoved == false) {
    if (xValue < minInputThreshold) {
      ...
      if (matrix[playerX - 1][playerY] == SPACE && playerX - 1 >= 0) {
        ...
        if (doMainButtonAction == true && (dirtBomb == false || (dirtBomb == true && numberOfDirtBombs > 0))) {
